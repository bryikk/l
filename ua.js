  ...
  function component(object) {
    ...
    this.startSource = function(json) {
      return new Promise(function(resolve, reject) {
        ...
        // ПРИМУСОВО ВШИВАЄМО UAFIX ЯК ПОВНОЦІННЕ ДЖЕРЕЛО BWA
        sources['uafix'] = {
      ...
      return new Promise(function(resolve, reject) {
        ...
        var fin = function fin(call) {
          ...
          network.silent(account(url), function(json) {
            ...
            // ТАК САМО ДОДАЄМО ТУДИ В LIFE-ПОДІЇ БАЛАНСЕРА
    ...
    // НАШ АВТОНОМНИЙ ЛОКАЛЬНИЙ ПАРСЕР ДЛЯ UAFIX.NET ВСЕРЕДИНІ BWA
    this.uafixSearchLocal = function() {
      ...
      $.ajax({
        ...
        success: function(html) {
          ...
          dom.find('a[href*="/video/"], a[href*="/films/"], .shortstory a, .movie-item a').each(function(index) {
    ...
    this.getFileUrl = function(file, call, waiting_rch) {
      ...
      if (file.url && file.url.indexOf('uafix.net') !== -1) {
          // Залізна логіка витягування потоку uafix
          var corsProxy = "https://api.allorigins.win/raw?url=";
              ...
              success: function(html) {
                  ...
                  if (match && match[1]) {
                      ...
                      if (src.indexOf('.m3u8') !== -1) {
                          Lampa.Loading.stop();
                          call({ method: 'play', url: src }, { url: src });
                      } else {
                          // Лліземо в iframe плеєра
                          $.ajax({
                              ...
                              success: function(iframeHtml) {
                                  ...
                                  if (fileMatch && fileMatch[1]) {
                                      call({ method: 'play', url: fileMatch[1] }, { url: fileMatch[1] });
                                  } else { call(false, {}); }
                              },
  ...
  function startPlugin() {
    ...
    if (Lampa.Manifest.app_digital >= 177) {
        var balansers_sync = [
            ...
            "mikai",
            "dreamerscast",
            "uafix" // Синхронізація налаштувань для нашого балансера
        ];
